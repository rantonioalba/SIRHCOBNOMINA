...
//  SIRHCOBNOMINA
//
//  Created by Roberto Antonio on 19/05/21.
//

import Foundation

struct SearchService {
    static func fetch(token:String, search:String, completionHandler: @escaping(_ paySheet: MAuthenticate<MPaySheet>?, _ error:String?)->Void)  {
        
        var objects:MAuthenticate<MPaySheet>?
        
        let url = NSURL(string: "http://54.184.232.181:3000/nominas/\(search)" as String)
